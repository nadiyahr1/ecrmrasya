...
$stmt = $conn->prepare("SELECT m.*, l.diskon as diskon_level, l.nama_level FROM tb_member m JOIN tb_level_member l ON m.id_level = l.id_level WHERE m.id_member = ?");
$stmt->execute([$id_m]);
$user = $stmt->fetch();

$ada_fasilitas = !empty($_SESSION['keranjang_fasilitas']);
?>

<div style="padding: 20px; display: flex; gap: 30px; align-items: flex-start; background: #fdfdfd; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
    ...
    <h2>Checkout</h2>
        <section style="margin-bottom: 30px;">
            ...
            <div style="margin-left: 35px;"> <strong><?= $user['nama_member'] ?></strong> (Member <?= $user['nama_level'] ?>) <br>
        ...
        <section style="margin-bottom: 30px;">
            ...
            <div style="margin-left: 35px;"> <?php 
                ...
                if(!empty($_SESSION['keranjang'])):
                    foreach($_SESSION['keranjang'] as $id => $q):
                        $m = $conn->query("SELECT * FROM tb_menu WHERE id_menu = $id")->fetch();
                        $sub_m += ($m['harga'] * $q); $qty_m += $q;
                ?>
                    <div style="display: flex; align-items: center; margin-bottom: 15px; border-bottom: 1px dashed #eee; padding-bottom: 10px;">
                        <img src="assets/img/menu/<?= $m['foto'] ?? 'default_menu.jpg' ?>" width="70" height="70" style="border-radius: 10px; object-fit: cover; margin-right: 15px; border: 1px solid #eee;">
                        <div style="flex: 1;">
                ...
                ?>
                    <div style="display: flex; align-items: center; margin-bottom: 15px; border-bottom: 1px dashed #eee; padding-bottom: 10px;">
                        <img src="assets/img/fasilitas/<?= $f['foto_fasilitas'] ?? 'default_fas.jpg' ?>" width="70" height="70" style="border-radius: 10px; object-fit: cover; margin-right: 15px; border: 1px solid #eee;">
                        <div style="flex: 1;">
                            <strong>[Fasilitas] <?= $f['nama_fasilitas'] ?></strong><br>
                            <small><?= $b['tgl_booking'] ?> (<?= $b['jam_mulai'] ?>) | <?= $b['durasi'] ?> Jam</small>
                        </div>
                        <strong>Rp <?= number_format($f['harga_per_jam'] * $b['durasi']) ?></strong>
                    </div>
                <?php endforeach; endif; ?>
            </div>
        </section>
        ...
        <section style="margin-bottom: 30px;">
            ...
            <div style="margin-left: 35px;"> <?php if ($ada_fasilitas): ?>
                    ...
                    <input type="hidden" name="tipe_pesanan" value="Makan di Tempat">
                <?php else: ?>
                    <label><input type="radio" name="tipe_pesanan" value="Ambil di Cafe" checked onclick="toggleMeja(false)"> Ambil di Cafe (Pick-up)</label> <br>
                    <label><input type="radio" name="tipe_pesanan" value="Makan di Tempat" onclick="toggleMeja(true)"> Makan di Tempat (Dine-in)</label>
                <?php endif; ?>

                <div id="area_meja" style="margin-top: 10px; display: <?= $ada_fasilitas ? 'block' : 'none' ?>;">
                    <label>Pilih Nomor Meja:</label>
                    <select name="id_meja" style="padding: 5px;">
                        <?php 
                        $mejas = $conn->query("SELECT * FROM tb_meja WHERE status = 'Tersedia'")->fetchAll();
        ...
        <section style="margin-bottom: 30px;">
            ...
            <div style="margin-left: 35px; background: #f9f9f9; border-radius: 10px;">
    ...
    <div style="flex: 1; position: sticky; top: 20px;">
        <div style="background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border-top: 5px solid #6F4E37;">
            ...
            <form action="simpan_pesanan.php" method="POST">
                ...
                <input type="hidden" name="id_meja" id="id_meja_hidden" value="<?= $ada_fasilitas ? '1' : '' ?>"> 
...
<div id="vModal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.7); backdrop-filter: blur(3px);">
    <div style="background: white; margin: 8% auto; padding: 25px; width: 450px; border-radius: 15px; position: relative;">
        ...
        foreach($promos as $p):
            $cukup = ($user['poin'] >= $p['poin_dibutuhkan']);
        ?>
            <div style="padding: 15px; border: 2px solid #f0f0f0; border-radius: 12px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; opacity: <?= $cukup ? '1' : '0.5' ?>;">
                <div>
                    <strong style="color: #6F4E37;"><?= $p['nama_promo'] ?></strong> <br>
                    <small>Butuh <?= $p['poin_dibutuhkan'] ?> Poin | Potongan Rp <?= number_format($p['nominal_potongan']) ?></small>
                </div>
                <?php if($cukup): ?>
                    <button type="button" onclick="pilihV('<?= $p['id_promo'] ?>', '<?= $p['nama_promo'] ?>', <?= $p['nominal_potongan'] ?>)" style="background: #6F4E37; color: white; border: none; padding: 8px 12px; border-radius: 6px; cursor: pointer;">Tukar</button>
                <?php else: ?>
                    <button disabled style="padding: 8px 12px; border-radius: 6px; font-size: 11px;">Poin Kurang</button>
                <?php endif; ?>
            </div>
        <?php endforeach; ?>
        </div>
        <button onclick="closeVoucherModal()" style="width: 100%; margin-top: 15px; padding: 12px; border: none; background: #eee; border-radius: 8px; cursor: pointer;">Tutup</button>
    </div>
</div>
...
function toggleMeja(show) { document.getElementById('area_meja').style.display = show ? 'block' : 'none'; }
function toggleNote(cek) { document.getElementById('box_note').style.display = cek.checked ? 'block' : 'none'; }
function openVoucherModal() { document.getElementById('vModal').style.display = 'block'; }
function closeVoucherModal() { document.getElementById('vModal').style.display = 'none'; }

// Fungsi Pilih Voucher Loyalitas
function pilihV(id, nama, nominal) {
    document.getElementById('id_v_hidden').value = id;
    document.getElementById('v_nama').innerText = "Berhasil: " + nama + " (-Rp " + nominal.toLocaleString() + ")";
    document.getElementById('v_terpilih').style.display = 'block';
    
    document.getElementById('row_v').style.display = 'flex';
    document.getElementById('v_val_text').innerText = "-Rp " + nominal.toLocaleString();
    
    document.getElementById('total_akhir').innerText = "Rp " + (totalBase - nominal).toLocaleString();
    closeVoucherModal();
}

// Fungsi Cek Kode Kupon Umum (Demo Sederhana)
function cekKupon() {
    let kode = document.getElementById('kode_kupon').value;
    if(kode === "") { alert("Masukkan kode terlebih dahulu!"); return; }
    
    // Simulasi Cek ke Database (Untuk Sidang bisa dikembangkan ke AJAX)
...
function removeVoucher() {
    ...
    document.getElementById('total_akhir').innerText = "Rp " + totalBase.toLocaleString();
}
</script>
