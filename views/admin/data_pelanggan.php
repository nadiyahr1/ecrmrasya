...
<div class="stats-container">
    ...
    <div class="stat-card" style="border-left-color: #ffd700;">
        ...
        <h2><?= number_format($total_poin_beredar) ?> Poin</h2>
    </div>
</div>
...
<table class="table-pelanggan">
    ...
    <tbody>
        ...
        <?php if (count($members) == 0): ?>
            <tr>
                <td colspan="8" style="text-align: center; color: #888; padding: 40px;">Data pelanggan tidak ditemukan.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
...
<?php if ($total_halaman > 1): ?>
    <div style="display: flex; justify-content: center; margin-top: 20px; gap: 5px;">
        <?php for ($i = 1; $i <= $total_halaman; $i++): ?>
            <a href="index.php?controller=admin&action=data_pelanggan&search=<?= urlencode($search) ?>&halaman=<?= $i ?>"
...
<script>
    function lihatRiwayat(id, nama) {
        ...
        document.getElementById('isiModalMember').innerHTML = "<div style='text-align:center; color:#888;'><i>⏳ Mengambil data riwayat...</i></div>";

        // PERBAIKAN: Memanggil method riwayat_poin_pelanggan di AdminController
        fetch('index.php?controller=admin&action=riwayat_poin_pelanggan&id=' + id)
            ...
            .catch(err => {
                document.getElementById('isiModalMember').innerHTML = "<div style='color:red;'>Terjadi kesalahan saat memuat data.</div>";
            });
    }
