...
<div style="margin-top: 20px; display: flex; justify-content: space-between; align-items: center;">
    <div style="font-size: 14px; color: #666;">
        Menampilkan <?= count($promos) ?> dari <?= $total_data ?> promo
    </div>
    <div style="display: flex; gap: 5px;">
        <?php if ($halaman_aktif > 1): ?>
            <a href="index.php?controller=admin&action=promo&halaman=<?= $halaman_aktif - 1 ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>" style="padding: 8px 12px; border: 1px solid #ddd; border-radius: 5px; text-decoration: none; color: #333; background: white; font-size: 14px;">&laquo; Prev</a>
        <?php endif; ?>
        ...
        <?php if ($halaman_aktif < $total_halaman): ?>
            <a href="index.php?controller=admin&action=promo&halaman=<?= $halaman_aktif + 1 ?>&search=<?= urlencode($search) ?>&limit=<?= $limit ?>" style="padding: 8px 12px; border: 1px solid #ddd; border-radius: 5px; text-decoration: none; color: #333; background: white; font-size: 14px;">Next &raquo;</a>
        <?php endif; ?>
    </div>
</div>
